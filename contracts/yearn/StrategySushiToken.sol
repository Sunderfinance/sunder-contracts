...
contract StrategySushiToken {
    ...
    /*
    address constant public want   = address(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2); // Sushi
    address constant public xSushi = address(0x8798249c2E607446EfB7Ad49eC89dD1865Ff4272); // xSushi
    */
    address constant public want   = address(0xcC7675C0B1f4DDf3c774e960e4C627F566CB3f0F); // Sushi
    address constant public xSushi = address(0xbD57731106CD80924F13F57F6e6825c8375b07b2); // xSushi
    ...
    function _withdraw(address _receiver, uint256 _amount) internal {
        uint256 _balance = IERC20(want).balanceOf(address(this));
        if (_balance < _amount) {
            uint256  _share = getShare(_amount.sub(_balance));
            if (_share > 0) {
                ISushiBar(xSushi).leave(_share);
            }
    ...
    function withdrawAll(address _receiver) external returns (uint256 _balance) {
        require(msg.sender == controller, "!controller");
        uint256 _share = ISushiBar(xSushi).balanceOf(address(this));
        if (_share > 0) {
            ISushiBar(xSushi).leave(_share);
        }
        debt = 0;
    ...
    function earn() external {
        ...
        if (_balance > 0) {
            IERC20(want).approve(xSushi, _balance);
            ISushiBar(xSushi).enter(_balance);
        }
    }
    ...
    function balanceXSushiWant() public view returns (uint256 _amount) {
        uint256 _totalShares = ISushiBar(xSushi).totalSupply();
        if (_totalShares == 0) {
            _amount = 0;
        } else {
            uint256 _share = ISushiBar(xSushi).balanceOf(address(this));
            uint256 _balance = IERC20(want).balanceOf(xSushi);
            _amount = _share.mul(_balance).div(_totalShares);
        }
    }
    function totalAssets() public view returns (uint256) {
        return balanceWant().add(balanceXSushiWant());
    }
    ...
    function getShare(uint256 _amount) public view returns (uint256) {
        ...
        uint256 _totalShares = ISushiBar(xSushi).totalSupply();
        return _amount.mul(_totalShares).div(_balance);
    }
}
